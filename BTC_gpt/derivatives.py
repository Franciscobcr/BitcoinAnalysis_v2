    ...
    class options_volume:
        ...
        def analysis(self):
            ...
            if vol_negociacao is not None and not vol_negociacao.empty:
                # Converter a coluna 'volume' para float
                ...
                # Filtrar os instrumentos com maior volume
                top_instrument = vol_negociacao.loc[vol_negociacao['volume'].idxmax()]

                # Gerando a análise para o volume de negociação de opções de Bitcoin
    ...
    class cvd_data:
        ...
        def analysis(self):
            ...
            if not cvd_30min.empty:
                ...
                # Último CVD registrado
                last_cvd = cvd_30min['cvd'].iloc[-1]

                # Gerando a análise sobre o CVD
    ...
    class voume_change:   
        ...
        def analysis(self):
            ...
            if not mudanca_volume.empty:
                ...
                # Última mudança de volume registrada
                last_change_volume = mudanca_volume['change_in_volume'].iloc[-1]

                # Gerando a análise sobre a mudança de volume
    ...
    class iv:
        ...
        def analysis(self):
            ...
            if iv is not None and not iv.empty:
                # Converter a coluna 'mark_iv' para float
                ...
                # Gerando a análise para o indicador de volatilidade implícita
    ...
    class market_depth:
        ...
        def get_order_book_depth(self, symbol, exchange="Binance", interval="30m", limit=1000, start_time=None, end_time=None):
            ...
            if response.status_code != 200:
                ...
                return None
            data = response.json()
            return "Dados retornados pela API:", data 
    ...
    class funding_rate_vol:
        ...
        def analyze(self):
            ...
            if not vol_weight_ohlc.empty:
                ...
                return f"Nenhum dado foi retornado para análise. weight_ohlc_history"
